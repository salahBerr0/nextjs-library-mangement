...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setBorrowingForm(prev => ({ ...prev, dueDate: defaultDueDate.toISOString().split('T')[0] }));
  }, []);
  ...
  const addBook = async (e) => {
    ...
    try {
      ...
      setBookForm({ title: '', author: '', isbn: '', category: 'Fiction', copies: 1, coverImage: '', description: '' });
      loadBooks();
    } catch (error) {
  ...
  const updateBook = async (e) => {
    ...
    try {
      ...
      showAlert('Erreur lors de la mise à jour du livre: ' + error.message, 'error');
    }
  };
  ...
  const loadBorrowings = async () => {
    try {
      const q = query(collection(db, 'borrowings'), where('status', '==', "borrowed"));
  ...
  const activeMembers = members.filter(m => (m.activeBorrowings || 0) > 0).length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-100 via-neutral-50 to-stone-200 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Alert modernisé */}
        {alert.show && (
          <div className={`mb-6 p-4 rounded-xl shadow-md transform transition-all duration-300 ${
            alert.type === 'success' 
            ...
            <div className="flex items-center">
              <span className="text-2xl mr-3">{alert.type === 'success' ? '✅' : '⚠️'}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-gradient-to-br from-[#2B3A29] via-[#3F5D43] to-[#4E7C55] rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-stone-200 text-sm font-medium mb-1">Total Livres</p>
              ...
              <div className="bg-gradient-to-br from-[#2B3A29] via-[#3F5D43] to-[#4E7C55] rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-stone-200 text-sm font-medium mb-1">Disponibles</p>
              ...
              <div className="bg-gradient-to-br from-[#2B3A29] via-[#3F5D43] to-[#4E7C55]

] rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-stone-200 text-sm font-medium mb-1">Empruntés</p>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 bg-white rounded-2xl shadow-lg p-6 border border-stone-200">
                ...
                <form onSubmit={editingBook ? updateBook : addBook} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-2">Auteur *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-2">ISBN</label>
                    <input
                      type="text"
                      value={bookForm.isbn}
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-stone-700 mb-2">URL du Cover</label>
                    <input
                      type="url"
                    ...
                    {bookForm.coverImage && (
                      <div className="mt-4 flex justify-center">
                        <img 
                          ...
                          onError={(e) => e.target.src = 'https://via.placeholder.com/128x192?text=No+Image'}
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-stone-700 mb-2">Exemplaires *</label>
                      <input
                        type="number"
                        min="1"
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-6 border border-stone-200">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-stone-800">Bibliothèque</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-2">
                  {filteredBooks.map(book => (
                    <div key={book.id} className="bg-gradient-to-br from-stone-50 to-neutral-100 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-stone-200 p-4">
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-bold text-stone-800 text-lg mb-1 truncate">{book.title}</h3>
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${
                              ...
                              {book.available > 0 ? `✓ ${book.available} dispo` : '✗ Emprunté'}
                            </span>
                          </div>

                          {book.description && (
                            <p className="text-stone-500 text-xs line-clamp-2 mb-3">{book.description}</p>
                          )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-gradient-to-br from-[#2B3A29] via-[#3F5D43] to-[#4E7C55] rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-stone-200 text-sm font-medium mb-1">Total Membres</p>
              ...
              <div className="bg-gradient-to-br from-[#2B3A29] via-[#3F5D43] to-[#4E7C55] rounded-2xl shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-stone-200 text-sm font-medium mb-1">Emprunteurs Actifs</p>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-stone-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto pr-2">
                {filteredMembers.map(member => (
                  <div key={member.id} className="bg-gradient-to-br from-stone-50 to-neutral-100 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-stone-200 p-5">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-stone-800 text-lg">{member.username}</h3>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center justify-between">
                        <span className="text-stone-600 text-sm">Emprunts actifs:</span>
                    ...
                    <div className="flex gap-2">
                      {editingMember && editingMember.id === member.id ? (
                        <>
                          <button
                            onClick={async () => {
                              await updateMemberType(editingMember.id, editingMember.type);
